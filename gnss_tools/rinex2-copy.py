...
def parse_RINEX2_header(lines: List[str]):
    '''
    ------------------------------------------------------------
...
def parse_RINEX2_obs_data(lines: List[str], observations: List[str], century: int = 2000, exception_behavior = None):
    '''
    ------------------------------------------------------------
    ...
    while True:
        try:
            ...
            seconds = int(seconds)
            dt = np.datetime64(datetime(year, month, day, hour, minute, seconds, microseconds))
            time.append(dt)
            ...
            for sat_id in sat_ids:
                ...
                num_lines_per_sat = len(observations) // 5 + int(len(observations) % 5 > 0)
                line = ''
...
def transform_values_from_RINEX2_obs(rinex_data: Dict[str, Any], frequency_numbers: Dict[str, float] = None):
    '''
    ------------------------------------------------------------
...
def parse_RINEX2_obs_file(filepath: str, century: int = 2000, exception_behavior = None):
    '''
    ------------------------------------------------------------
    ...
    **Warning**: this function cannot currently handle splicing
    ...
    obs_data, time, epoch_flags = parse_RINEX2_obs_data(obs_lines, header['obs_types'], century, exception_behavior)
