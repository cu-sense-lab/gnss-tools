...
def generate_legendre_sequence() -> np.ndarray:
    legendre = np.ones(10223, dtype=np.int8)
    for i in range(10224):
        legendre[(i * i) % 10223] = -1
    legendre[0] = 1
    return legendre

def generate_L1CP_code_sequence(prn: int) -> np.ndarray:
    ...
    weilcode = np.ones(10223, dtype=np.int8)
    for i in range(10223):
        ind = (i + weil[prn - 1]) % 10223
