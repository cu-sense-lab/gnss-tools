...
import numpy as np
from gnss_lib.time.gmst import gpst2gmst, gpst2gmst_vec
...
def dms2deg(arr: np.ndarray):
    ...
    return (np.sign(arr[:, 0]) * (abs(arr[:, 0]) + arr[:, 1] / 60.0 + arr[:, 2] / 3600.0)).squeeze()


def ecf2geo(x_ref: NDArray[np.float64], timeout: Optional[int] = None, return_ROC: bool =False, tolerance: float =1e-12):
...
def local_enu(lat: float, lon: float):
    Rl = array([[-np.sin(lon),               np.cos(lon),             np.zeros((lat.shape[0],)) if isinstance(lat, np.ndarray) else 0],
                [-np.sin(lat) * np.cos(lon), -np.sin(lat) * np.sin(lon), np.cos(lat)],
                [ np.cos(lat) * np.cos(lon),  np.cos(lat) * np.sin(lon), np.sin(lat)]])
    return Rl


def ecf2enu(x_ref: NDArray[np.float64], x_obj: NDArray[np.float64], timeout: Optional[int]  =None):
...
def ecf2sky(x_ref: NDArray[np.float64], x_obj: NDArray[np.float64], timeout: Optional[int]=None):
...
from numpy import cos, sin, zeros, ones, array
...
def eci2ecf(
    ...
    R = array([[ np.cos(theta), -sin(theta), zeros(shp)],
               [ np.sin(theta),  np.cos(theta), zeros(shp)],
               [zeros(shp),  zeros(shp),  ones(shp)]])
    r_ecf = np.sum(R * r_eci.T[None, :, :], axis=1).T

    if v_eci is not None:
        dtheta = 7.29211585275553e-005  # Earth rotation [rad/s]
        Rdot = array([[-sin(theta), -cos(theta), zeros(shp)],
                      [ np.cos(theta), -sin(theta), zeros(shp)],
                      [ zeros(shp),  zeros(shp), zeros(shp)]]) * dtheta

        v_ecf = np.sum(R * v_eci.T[None, :, :], axis=1).T + np.sum(Rdot * r_eci.T[None, :, :], axis=1).T
...
def ecf2eci(
    ...
    if v_ecf is not None:
        ...
        v_eci = np.sum(R * v_ecf.T[None, :, :], axis=1).T + np.sum(Rdot * r_ecf.T[None, :, :], axis=1).T
