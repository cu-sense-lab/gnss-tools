...
def compute_sun_eci_coordinates(time: datetime) -> np.ndarray:
    ...
    lamb_ecli = lamb_sun + 1.914666471 * np.sin(np.radians(M_sun)) + 0.918994643 * np.sin(np.radians(2 * M_sun))  # ecliptic longit>
    dist = 1.000140612 - 0.016708617 * np.cos(np.radians(M_sun)) - 0.000139589 * np.cos(np.radians(2 * M_sun))  # distance to sun i>
